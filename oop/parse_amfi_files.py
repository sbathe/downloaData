...
class AmfiParse:
   ...
   def nodata(self,raw_string):
       ...
       if re.search(self.NODATA,string):
           return True
       else:
           return False
       ...
       for line in processed_data:
           if re.search(self.HEADING,line):
              ...
              amc = line
               ...
               data[amc][code]["meta"] = { "name": name, "type": "open
                                             ended", "categories": categories }
               if "data" in data[amc][code].keys():
                   data[amc][code]["data"].append([date,nav])
               else:
                   data[amc][code]["data"] = [date,nav]

"""
This still fails on assigining correct categories
def parse(processed_data):
     ...:        """ """The goal is:
     ...:            - Create a scheme code wise json structure with 2 sections:
     ...:            - meta: a dictionary that has scheme name, fund AMC, category etc
     ...:            - data: list of (date, NAV) tuples
     ...
     ...:                headings = line.split(';')
     ...:            elif re.search(OPENENDED,line):
     ...:               parts = line.split('(')
     ...:               for p in parts:
     ...:                   c.append(p.split('-'))
     ...:                   categories = [item.strip(')').strip() for sublist in c for item in sublist]
     ...:            elif re.search(AMC,line):
     ...:               amc = line
     ...:               data[amc] = {}
     ...:            elif len(line.split(';')) > 1:
     ...:                code,name,nav,rp,sp,date = line.split(';')
